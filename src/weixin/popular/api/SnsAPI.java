...
public class SnsAPI extends BaseAPI{
	...
	public static SnsToken oauth2AccessToken(String appid,String secret,String code){
		SnsToken snsToken = AuthCodeCache.getCodeAuthCache(code);
		if(snsToken==null) {
			...
			snsToken = LocalHttpClient.executeJsonResult(httpUriRequest, SnsToken.class);

			AuthCodeCache.putCodeAuthCache(code,snsToken);
		}
		return snsToken;
	}
	...
	public static SnsToken oauth2RefreshToken(String appid,String refresh_token){
		HttpUriRequest httpUriRequest = RequestBuilder.post()
				.setHeader("context-type","application/json;Charset=utf-8")
	 ...
	 * @return
	 */
	public static String qROauth2Authorize(String appid,String redirect_uri,boolean snsapi_login,String state){
		try {
			StringBuilder sb = new StringBuilder();
			sb.append(OPEN_URI + "/connect/oauth2/qrconnect?")
					.append("appid=").append(appid)
					.append("&redirect_uri=").append(URLEncoder.encode(redirect_uri, "utf-8"))
					.append("&response_type=code")
					.append("&scope=").append(snsapi_login?"snsapi_login":"snsapi_base")
