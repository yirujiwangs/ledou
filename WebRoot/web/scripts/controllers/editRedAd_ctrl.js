(function(){
    ...
    angular
        .module('center')
        .controller('EditRedAdCtrl', EditRedAdCtrl);

    EditRedAdCtrl.$inject =  [ '$scope', '$rootScope','base64Service','AdService','AdDao','dataService','$modal','utilService','$location','AlertService','$http','$filter','$timeout'];

    function EditRedAdCtrl($scope,$rootScope,base64Service,AdService,AdDao,dataService,$modal,utilService,$location,AlertService,$http,$filter,$timeout){
        ...
        if(adData != undefined){
            ...
            $scope.price=adData.red_price;
            $scope.probability=adData.probability;
            $scope.withdrawalId=adData.withdrawal.id;
            $scope.preFee= adData.withdrawal.once_money;
            AdDao.advStrategyDetail({advUUID:adData.advUUID,type:'surprise'})
                .then(function(res){
