(function(){
    ...
    angular
        .module('center')
        .controller('AppendAdCtrl', AppendAdCtrl);

    AppendAdCtrl.$inject = [ '$scope', '$rootScope','AdService','AdDao','base64Service','dataService','$modal','utilService','$location','AlertService','$http'];

    function AppendAdCtrl($scope,$rootScope,AdService,AdDao,base64Service,dataService,$modal,utilService,$location,AlertService,$http){
        var vm=this;
       ...
       //配置区域分组
        ...
        vm.setAdAllowance=function(){
            if(! $scope.adAllowance)$scope.adAllowance=0;
            var adAllowance=angular.element('#adAllowance');
            var o=adAllowance.val();
            if(o.toString().length==1)return;
            else if(o.toString().length>=1&&o.toString().charAt(0)=='0'){
                var n= o.slice(1);
                adAllowance.val(Number(n))
            }
        }

        $scope.preFeeChange = function(){
            $scope.withdrawalData = angular.fromJson($scope.withdrawal);
            //console.log(angular.fromJson($scope.withdrawal));
            //$scope.preFee =$scope.withdrawalData.once_money;
        };
        ...
        vm.submit = function(){
            ...
            //因为都是必填的所以这里不用再判断
            param.preFee = ($scope.touNum *$scope.preFee+$scope.adAllowance * $scope.touNum).toFixed(2);
            param.subsidy=parseInt(($scope.adAllowance.toFixed(2))*100);
